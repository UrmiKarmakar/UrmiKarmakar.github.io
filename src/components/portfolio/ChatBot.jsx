...
export default function ChatBot() {
  ...
  const [messages, setMessages] = useState([
    { 
      role: "assistant", 
      content: "Hey there, I'm **Urmi_AI**. Ready to gossip about tech or walk you through Urmi's amazing portfolio? ✨" 
    }
  ]);
  ...
  return (
    <div className="fixed bottom-0 right-0 w-full md:w-auto h-auto z-[9999] p-4 md:p-6 pointer-events-none flex flex-col items-end">
            ...
            <div className="relative z-10 p-5 border-b border-purple-500/20 bg-purple-950/40 backdrop-blur-md flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="relative">
                  ...
                  {/* Status Dot for Header Icon */}
                  <span className="absolute -top-1 -right-1 w-3.5 h-3.5 bg-green-500 rounded-full border-[3px] border-[#0f071a] shadow-[0_0_10px_#22c55e] animate-pulse"></span>
                </div>
                <div>
                  <h3 className="text-white font-bold text-base tracking-tight leading-none">Urmi_AI</h3>
                  {/* Personality Subtitle with Gradient */}
                  <p className="text-[10px] font-black uppercase tracking-[0.15em] bg-gradient-to-r from-green-400 via-purple-300 to-blue-400 bg-clip-text text-transparent animate-gradient-x">
                    Your Tech-Savvy Bestie ✨ 
                  </p>
                </div>
              </div>
      ...
      {/* Toggle Button (The design you liked) */}
      {!isOpen && (
        <motion.button
        ...
        >
          ...
          {/* Status Dot for Floating Button */}
          <span className="absolute top-1 right-1 w-3.5 h-3.5 bg-green-500 rounded-full border-2 border-[#0f071a] shadow-[0_0_8px_#22c55e] z-20 animate-pulse"></span>
        </motion.button>
      )}
    </div>
  );
